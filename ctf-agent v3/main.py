...
def test_api_key(llm_service: str, api_key: str) -> bool:
    ...
    try:
        if llm_service == "deepseek":
            ...
            from langchain_openai import ChatOpenAI
            llm = ChatOpenAI(
                openai_api_base="https://llmxapi.com/v1",  # 修复：使用正确的 Gemini 端点
                openai_api_key=api_key,
...
def handle_flag_verification(agent, challenge_data):
    ...
    if hasattr(agent, 'last_flag_candidate') and agent.last_flag_candidate:
        ...
        print(f"✅ Using CTF Agent detected flag: {candidate}")
        ...
        while agent.current_round < max_rounds:
            ...
            if choice == "1":
                ...
                if result == True:
                    ...
                    print("🔄 Continuing with solving process...")
                    continue
                else:
                    # No flag found, continue solving
                    print("🔄 No flag found, continuing with solving process...")
                    continue
            elif choice == "3":
            ...
            if hasattr(agent, 'last_tool_results') and agent.last_tool_results:
                ...
                # 🔧 确保每轮后都询问用户，即使没有tool results
                print(f"\n✅ Round {agent.current_round} completed successfully!")
                print("🔄 Asking user for next action...")
