...
class CTFPractice:
    ...
    def _manual_input_challenge(self) -> bool:
        ...
        print("Challenge Description (press Enter twice to finish):")
        description_lines = []
        empty_line_count = 0
        while True:
            line = input()
        ...
        cat_input = input(f"\nSelect category (1-{len(categories)}) [General Skills]: ").strip()
        if not cat_input:
        ...
        while True:
            ...
            # Strategy 1: Use the path as-is (absolute or relative to current directory)
            if os.path.exists(file_path):
                ...
                if relative_path.exists():
                    ...
                    # Strategy 3: Try just the filename in project root
                    filename_only = Path(file_path).name
                    filename_path = project_root / filename_only
                    if filename_path.exists():
                        resolved_path = str(filename_path)
            
            if resolved_path:
                attachments.append(resolved_path) 
                print(f"Added: {resolved_path}")
            else:
                print(f"File not found: {file_path}")
                print("Tip: Try using just the filename (e.g., 'chall.c') or relative path")
            ...
            try:
                ...
                if 1 <= choice <= len(challenges):
                    ...
                    if not normalized_files:
                        guessed = []
                        # First try challenge directory
                        for p in base_dir.iterdir():
                            if p.is_file() and p.suffix not in {'.json'}:
                                guessed.append(str(p))
                        
                        # If still empty, try project root directory for common challenge files
                        if not guessed:
                            project_root = Path('.')
                            for p in project_root.iterdir():
                                if p.is_file() and p.suffix not in {'.json', '.py', '.md', '.txt'} and not p.name.startswith('.'):
                                    guessed.append(str(p))
                        
                        selected_challenge['files'] = guessed
                    
                    # Verify that all files exist and are accessible
                    verified_files = []
                    for file_path in selected_challenge['files']:
                    ...
                    if verified_files:
                        selected_challenge['files'] = verified_files
                        print(f"Verified files: {', '.join(verified_files)}")
                    else:
                        print("Warning: No verified files found for this challenge")
