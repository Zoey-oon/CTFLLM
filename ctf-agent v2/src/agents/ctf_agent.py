    ...
    def _setup_llm(self):
        ...
        if self.llm_service == "deepseek":
            ...
            return ChatAnthropic(
                api_key=self.api_key,
                model="claude-sonnet-4-20250514",  # Latest Claude Sonnet 4 (May 2025)
                temperature=0.7,
                timeout=120,
                max_retries=3
            )
    ...
    def interact(self, user_input: str, input_source: str = "agent") -> str:
        ...
        try:
            ...
            if estimated_tokens > 120000:  # Leave some buffer for response
                ...
                print("   - Applying aggressive content reduction...")
                ...
                # Clear chat history more aggressively if needed
                chat_messages = self.memory.chat_memory.messages
                if len(chat_messages) > 5:  # Keep only last 5 messages for problematic cases
                    print(f"   - Clearing old chat history (keeping last 5 of {len(chat_messages)} messages)")
                    self.memory.chat_memory.messages = chat_messages[-5:]
                ...
                if estimated_tokens_after > 125000:  # Still too large
                    # Create a summary instead of sending everything
                    user_input = f"""CONTEXT TOO LARGE - SUMMARIZED:
The previous context exceeded token limits. Key information:
- Challenge: {getattr(self, 'challenge_info', {}).get('title', 'Unknown')}
- Current round encountered context overflow
- Please work with available tools to analyze files directly
- Focus on the immediate task rather than reviewing full history

{user_input[:50000]}

... (rest of content truncated - use tools for detailed analysis)"""
                    print(f"   - Created summary version: {len(user_input):,} characters")
