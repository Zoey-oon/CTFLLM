...
<tool>code_executor</tool><input>print("Hello World")</input>
<tool>code_executor</tool><input>bytes.fromhex('48656c6c6f').decode()</input>

DO NOT use JSON format like {"code": "..."} - just pass the raw Python code directly!
...
Remember to print() results to see output!"""
    ...
    def _run(self, code: str) -> str:
        ...
        try:
            ...
            # Debug mode
            import os
            debug_mode = os.getenv('CTF_DEBUG', '').lower() in ['true', '1', 'yes']
            ...
            if code.strip().startswith('{') and '"code"' in code:
                try:
                    ...
                    if 'code' in data:
                        code = data['code']
                        if debug_mode:
                            print(f"DEBUG CodeExecutor: Extracted code from JSON: {repr(code[:100])}")
                except:
                    pass  # Not valid JSON, continue with original code
            ...
            # Debug: check code before execution
            if debug_mode:
                print(f"DEBUG CodeExecutor: About to execute code: {repr(code[:100])}")
            ...
            try:
                ...
                if debug_mode:
                    print(f"DEBUG CodeExecutor: Captured output: {repr(result)}, length: {len(result)}")
                ...
                sys.stdout = original_stdout
                if debug_mode:
                    print(f"DEBUG CodeExecutor: Exception during execution: {e}")
                return json.dumps({
                    "success": False,
    ...
    def _preprocess_code(self, code: str) -> str:
        """Preprocess code to handle common LLM formatting issues"""
        code = code.strip()
        
        # Handle case where LLM wraps code in a string variable
        import re
        
        # Pattern: code = """...""" or code = '''...'''
        pattern = r'^code\s*=\s*["\'"]{3}(.*?)["\'"]{3}$'
        match = re.match(pattern, code, re.DOTALL)
        if match:
            # Extract the actual code from the string
...
class CTFAgent:
    ...
    def _setup_llm(self):
        ...
        if self.llm_service == "deepseek":
            ...
            return ChatOpenAI(
                # api_key=self.api_key,
                openai_api_base="https://llmxapi.com/v1", 
                openai_api_key=self.api_key,
...
- <tool>qrcode_reader</tool><input>{"image_path": "challenge.png"}</input>"""
        ...
        try:
            from src.agents.tools.package_installer import PackageInstallerTool
            if PackageInstallerTool is not None:
    ...
    def _create_agent(self):
        ...
        # 🔧 在system prompt中添加可用工具列表
        available_tools_info = "\n\nCURRENTLY AVAILABLE TOOLS:\n"
        for i, tool in enumerate(self.tools, 1):
            available_tools_info += f"{i}. {tool.name}\n"
        available_tools_info += "\nIMPORTANT: Only use the tools listed above. Do NOT use 'bash', 'str_replace_editor', or any other tools not in this list.\n"
        
        enhanced_system_prompt = system_prompt + available_tools_info
        ...
        try:
            ...
            if debug_mode:
                print(f"⚠️  create_openai_functions_agent failed: {e}")
                print("🔄 Trying alternative agent creation method...")
            
            try:
                # 备选方法1：使用create_structured_chat_agent
                ...
                if debug_mode:
                    print(f"⚠️  create_structured_chat_agent failed: {e2}")
                try:
                    # 备选方法2：使用create_react_agent
                    ...
                    if debug_mode:
                        print(f"⚠️  create_react_agent failed: {e3}")
                    # 最后的备选方案：创建简单的工具链
                    from langchain.chains import LLMChain
                    agent = LLMChain(llm=self.llm, prompt=prompt)
                    if debug_mode:
                        print("✅ Using simple LLMChain as fallback")
    ...
    def start_challenge(self, challenge_data: Dict) -> str:
        ...
        try:
            ...
            # Check if we're already in a challenge directory
            cwd = os.getcwd()
            if f"challenges{os.sep}{year}{os.sep}{category}{os.sep}{title_slug}" in cwd:
                # We're already in the challenge directory, use current directory
                cdir = Path(".")
                task_tree_filename = f"task_tree_{self.mode}.json"
                task_tree_path = cdir / task_tree_filename
            else:
                # Create challenge directory path from root
        ...
        available_files = []
        try:
            # Get challenge files from challenge data first
            ...
            # If no challenge files found from metadata, scan current directory
            if not available_files:
                current_files = [f for f in os.listdir('.') if os.path.isfile(f)]
                # Exclude obvious project files
                exclude_patterns = {'auto_', 'task_tree', '.json', 'requirements.txt', 'main.py', 'src', '__pycache__'}
                
                for file in current_files:
                    # Include any file that doesn't match exclude patterns
        ...
        # Better messaging for challenges without files
        if not available_files:
            # Check if this is a network/SSH challenge
            challenge_desc = challenge_data.get('description', '').lower()
            if 'ssh' in challenge_desc or 'connect' in challenge_desc or 'server' in challenge_desc:
                files_info = "  (no local files - this appears to be a network/SSH challenge)"
            else:
                files_info = "  (no challenge files found)"
        else:
            files_info = "\n".join(available_files)
        
        # 检查是否有ZIP文件被处理
        zip_processing_info = ""
        if any(f.lower().endswith('.zip') for f in challenge_data.get('files', [])):
            zip_processing_info = "\nZIP files detected and automatically processed!"
            zip_processing_info += "\n   - ZIP contents have been extracted and analyzed"
            zip_processing_info += "\n   - Extracted files are available in 'extracted/' directory"
            zip_processing_info += "\n   - File contents are shown below for immediate analysis"
        ...
        if files_content:
            ...
            if content_size > 15000:  # 降低到15KB限制，避免prompt过长
                if debug_mode:
                    print(f"⚠️  File content too large ({content_size:,} chars), truncating...")
                files_content = files_content[:15000] + "\n\n... (file content truncated - use tools to read specific files for detailed analysis)"
                if debug_mode:
        ...
        # 在initial prompt中添加可用工具信息
        available_tools_info = "\nAVAILABLE TOOLS:\n"
        for i, tool in enumerate(self.tools, 1):
            available_tools_info += f"{i}. {tool.name}\n"
        available_tools_info += "Only use the tools listed above, Do NOT use any other tools not in this list.\n"
        
        # 优化prompt结构，将文件内容放在最后，避免过长
        ...
        # Test tool functionality (only in debug mode)
        if debug_mode:
    ...
    def interact(self, user_input: str, input_source: str = "agent") -> str:
        ...
        try:
            ...
            if estimated_tokens > 120000:  # Leave some buffer for response
                print(f"⚠️  Content too large ({estimated_tokens:,} tokens), applying reduction...")
                
                # Identify what's causing the length issue
                chat_messages = self.memory.chat_memory.messages
                message_sizes = []
                for i, msg in enumerate(chat_messages):
                    if hasattr(msg, 'content'):
                        msg_size = len(str(msg.content))
                        message_sizes.append((i, msg_size))
                
                # Sort by size to find the largest messages
                message_sizes.sort(key=lambda x: x[1], reverse=True)
                
                print(f"   - Chat history analysis:")
                for i, (msg_idx, size) in enumerate(message_sizes[:3]):  # Show top 3 largest
                    print(f"     Message {msg_idx}: {size:,} chars")
                
                # 🔧 智能内容优化策略 - 保留必要内容
                if len(user_input) > 80000:  # 80KB limit for user input
                    # 保留开头和结尾的重要信息
                    keep_start = 40000  # 保留前40KB
                    keep_end = 20000    # 保留后20KB
                    
                    if len(user_input) > keep_start + keep_end:
                        user_input = (user_input[:keep_start] + 
                                    "\n\n... (middle content optimized for length) ...\n\n" +
                                    user_input[-keep_end:])
                        print(f"   - Input optimized: kept {keep_start:,} + {keep_end:,} chars")
                ...
                if len(chat_messages) > 5:
                    ...
                    # 策略1：保留最后3条消息（最近的上下文）
                    essential_messages.extend(chat_messages[-3:])
                    
                    # 策略2：保留包含flag的消息（高价值）
                    flag_messages = []
                    for i, msg in enumerate(chat_messages):
                        if hasattr(msg, 'content') and 'picoCTF{' in str(msg.content):
                            flag_messages.append((i, msg))
                    
                    # 策略3：保留包含工具执行结果的消息（高价值）
                    tool_messages = []
                    for i, msg in enumerate(chat_messages):
                        if hasattr(msg, 'content') and any(tool in str(msg.content) for tool in ['<tool>', 'Tool:', 'Result:']):
                            tool_messages.append((i, msg))
                    
                    # 策略4：保留包含任务状态更新的消息（高价值）
                    task_messages = []
                    for i, msg in enumerate(chat_messages):
                        if hasattr(msg, 'content') and '→ Task:' in str(msg.content):
                            task_messages.append((i, msg))
                    
                    # 合并重要消息，避免重复
                    important_indices = set()
                    for msg_list in [flag_messages, tool_messages, task_messages]:
                        for idx, msg in msg_list:
                            if idx not in important_indices and idx not in [len(chat_messages)-3, len(chat_messages)-2, len(chat_messages)-1]:
                                important_indices.add(idx)
                                essential_messages.insert(0, msg)
                    
                    # 如果重要消息太少，添加一些中等大小的消息
                    if len(essential_messages) < 5:
                        medium_messages = []
                        for i, msg in enumerate(chat_messages):
                            if hasattr(msg, 'content'):
                                msg_size = len(str(msg.content))
                                if 1000 < msg_size < 10000 and i not in important_indices:  # 中等大小
                                    medium_messages.append((i, msg))
                        
                        # 按时间顺序添加中等消息
                        medium_messages.sort(key=lambda x: x[0])
                        for idx, msg in medium_messages[:3]:  # 最多添加3条
                            if len(essential_messages) < 8:  # 总消息数限制
                                essential_messages.insert(0, msg)
                    
                    self.memory.chat_memory.messages = essential_messages
                    print(f"   - Smart memory optimization: {len(essential_messages)} essential messages")
                    print(f"   - Preserved: {len(flag_messages)} flag messages, {len(tool_messages)} tool messages, {len(task_messages)} task messages")
                
                # 重新检查总内容长度
                total_content_after = user_input
                for msg in self.memory.chat_memory.messages:
                    if hasattr(msg, 'content'):
                        total_content_after += str(msg.content)
                
                estimated_tokens_after = len(total_content_after) // 4
                
                if estimated_tokens_after > 125000:  # 仍然太大
                    # 创建智能摘要，保留关键信息
                    challenge_title = getattr(self, 'challenge_info', {}).get('title', 'Unknown')
                    current_round = getattr(self, 'current_round', 'Unknown')
                    
                    # 保留用户输入的核心部分
                    core_input = user_input[:60000]  # 保留60KB核心内容
                    
                    summary_prompt = f"Challenge: {challenge_title}, Round: {current_round}\n"
                    summary_prompt += "Content optimized for length. Core information:\n\n"
                    summary_prompt += core_input
                    summary_prompt += "\n\n... (content optimized for token limit)"
                    
                    user_input = summary_prompt
                    print(f"   - Final optimization: {len(user_input):,} characters")
                    
                    # 清理聊天历史，只保留当前轮次
                    self.memory.chat_memory.messages = []
                    print(f"   - Cleaned chat history for fresh start")
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                response = {"output": llm_response.content}
                            except Exception as e4:
                                # 创建一个简单的响应来继续
                                response = {
                                    "output": "I understand the challenge. Let me analyze the files and start solving step by step. First, I'll examine the extracted image file for hidden data using steganography tools."
                                }
                else:
                    # 其他错误，重新抛出
                    raise e
    ...
    def get_conversation_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "model": {
                ...
                # "api_base": self._get_api_base_info()
            },
    ...
    def extract_flag_from_conversation(self) -> Optional[str]:
        ...
        # 增强：检测hex编码的flag
        for round_record in reversed(self.conversation_history):
            # 检查AI响应中的hex字符串
            hex_pattern = r'[0-9a-fA-F]{20,}'  # 至少20个hex字符
            hex_matches = re.findall(hex_pattern, round_record.ai_response)
            ...
            for hex_str in hex_matches:
                ...
                try:
                    ...
                    # 检查解码后的字符串是否包含flag
                    if 'picoCTF{' in decoded or 'flag' in decoded.lower():
                        return f"hex_encoded: {hex_str} -> {decoded}"
                except:
                    continue
            
            # 检查human input中的hex字符串
            hex_matches = re.findall(hex_pattern, round_record.human_input)
            for hex_str in hex_matches:
                try:
                    decoded = bytes.fromhex(hex_str).decode('utf-8', errors='ignore')
                    if 'picoCTF{' in decoded or 'flag' in decoded.lower():
    ...
    def determine_next_input(self, challenge_data: Dict, summary: Dict) -> str:
        ...
        # 🔧 修复模板变量替换
        if results:
            # 如果有工具结果，替换模板中的{results}变量
            if isinstance(results, list):
                results_text = "\n".join([str(r) for r in results if r])
            else:
                results_text = str(results)
            base_prompt = base_prompt.replace("{results}", results_text)
        else:
            # 如果没有结果，替换为默认信息
            base_prompt = base_prompt.replace("{results}", "No previous results available. Starting fresh.")
        ...
        for zip_file in zip_files:
            try:
                ...
                if not os.path.exists(zip_path):
                    ...
                    if not os.path.exists(zip_path):
                        # 尝试在当前工作目录查找
                        zip_path = os.path.join(os.getcwd(), os.path.basename(zip_file))
                        if not os.path.exists(zip_path):
                            # 最后尝试在challenge目录查找
                            zip_path = os.path.join(os.getcwd(), zip_file)
                
                print(f"🔍 Auto-processing ZIP file: {zip_file}")
                print(f"   - Resolved path: {zip_path}")
                print(f"   - Path exists: {os.path.exists(zip_path)}")
                
                if not os.path.exists(zip_path):
                    file_contents += f"=== ZIP FILE: {os.path.basename(zip_file)} ===\n"
                    file_contents += f"Status: File not found\n"
                    file_contents += f"Error: Could not locate ZIP file at any of these paths:\n"
                    file_contents += f"   - {zip_file}\n"
                    file_contents += f"   - {os.path.basename(zip_file)}\n"
                    file_contents += f"   - {os.path.join(os.getcwd(), os.path.basename(zip_file))}\n"
                    file_contents += f"   - {os.path.join(os.getcwd(), zip_file)}\n"
                    file_contents += f"Current working directory: {os.getcwd()}\n\n"
                    continue
                ...
                if result.get('success'):
                    # 添加ZIP文件信息
                    file_contents += f"=== ZIP FILE: {os.path.basename(zip_file)} ===\n"
                    file_contents += f"Status: Successfully extracted and analyzed\n"
                    
                    # 添加解压后的文件内容
                    if result.get('file_contents'):
                        file_contents += f"Extracted text files:\n"
                        for file_path, content_info in result['file_contents'].items():
                            file_contents += f"\n--- {file_path} ---\n"
                            if 'error' in content_info:
                                file_contents += f"Error: {content_info['error']}\n"
                            else:
                                file_contents += f"Size: {content_info.get('size', 0)} bytes\n"
                                if content_info.get('content'):
                                    # 限制内容长度防止token溢出
                                    content = content_info.get('content')
                                    if len(content) > 3000:
                                        content = content[:3000] + f"\n... (truncated, full content available via file_reader tool)"
                                    file_contents += f"Content:\n{content}\n"
                                else:
                                    file_contents += "Binary file\n"
                    
                    # 添加二进制文件列表
                    if result.get('binary_files'):
                        file_contents += f"\nExtracted binary files:\n"
                        for binary_file in result['binary_files']:
                            file_contents += f"- {binary_file}\n"
                    
                    # 添加提取位置信息
                    extract_dir = f"extracted/{os.path.splitext(os.path.basename(zip_file))[0]}"
                    file_contents += f"\nExtraction location: {extract_dir}/\n"
                    file_contents += f"Note: All extracted files are available in the '{extract_dir}' directory\n\n"
                    
                else:
                    file_contents += f"=== ZIP FILE: {os.path.basename(zip_file)} ===\n"
                    file_contents += f"Status: Extraction failed\n"
                    file_contents += f"Error: {result.get('error', 'Unknown error')}\n\n"
                    
            except Exception as e:
                file_contents += f"=== ZIP FILE: {os.path.basename(zip_file)} ===\n"
                file_contents += f"Status: Processing error\n"
    ...
    def _get_api_base_info(self):
        # Implement your logic to get API base info here
