    ...
    def _setup_llm(self):
        ...
        if self.llm_service == "deepseek":
            ...
            return ChatAnthropic(
                # api_key=self.api_key,
                anthropic_api_url="https://xiaoai.plus",  # Custom API endpoint (without /v1)
    ...
    def interact(self, user_input: str, input_source: str = "agent") -> str:
        ...
        try:
            ...
            if estimated_tokens > 120000:  # Leave some buffer for response
                ...
                print("   - Applying intelligent content reduction...")
                ...
                # Keep only essential messages (last 3 + largest ones if they're important)
                if len(chat_messages) > 3:
                    ...
                    # If we have very large messages, keep the most recent large one
                    if message_sizes and message_sizes[0][1] > 50000:  # If largest message > 50KB
                        largest_msg_idx = message_sizes[0][0]
                        if largest_msg_idx not in [len(chat_messages)-3, len(chat_messages)-2, len(chat_messages)-1]:
                            # Add the largest message if it's not already in the last 3
                            essential_messages.insert(0, chat_messages[largest_msg_idx])
                            print(f"   - Kept large message {largest_msg_idx} ({message_sizes[0][1]:,} chars)")
                    
                    self.memory.chat_memory.messages = essential_messages
                    print(f"   - Reduced chat history from {len(chat_messages)} to {len(essential_messages)} messages")
                
                # Recheck total content after truncation
                ...
                print(f"   - Content after reduction: {estimated_tokens_after:,} tokens")
                
                if estimated_tokens_after > 125000:  # Still too large
                    # Create a focused summary with key information
                    challenge_title = getattr(self, 'challenge_info', {}).get('title', 'Unknown')
                    current_round = getattr(self, 'current_round', 'Unknown')
                    
                    # Analyze what caused the overflow
                    overflow_analysis = []
                    if len(user_input) > 50000:
                        overflow_analysis.append(f"- User input: {len(user_input):,} chars (truncated to 40K)")
                    
                    large_messages = [size for _, size in message_sizes if size > 10000]
                    if large_messages:
                        overflow_analysis.append(f"- Large chat messages: {len(large_messages)} messages >10K chars")
                        overflow_analysis.append(f"- Largest message: {max(large_messages):,} chars")
                    
                    overflow_analysis.append(f"- Total estimated: {estimated_tokens:,} tokens")
                    
                    user_input = f"""CONTEXT TOO LARGE - INTELLIGENT SUMMARY:
Challenge: {challenge_title} | Round: {current_round}

Previous context exceeded token limits. Here's what happened:
{chr(10).join(overflow_analysis)}
- Main causes: Large tool outputs or chat history
- Solution: Focus on immediate task, use tools directly

Key information from current input:
{user_input[:40000]}

... (content truncated - use tools for detailed analysis)"""
                    print(f"   - Created intelligent summary: {len(user_input):,} characters")
                    
                    # Clear all chat history to prevent further overflow
                    self.memory.chat_memory.messages = []
                    print(f"   - Cleared all chat history to prevent further overflow")
